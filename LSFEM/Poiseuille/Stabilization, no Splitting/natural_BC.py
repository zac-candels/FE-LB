...
for idx in range(Q):
    ...
    lin_form_vol_int = ( alpha_minus*alpha_plus*f_n[idx]*v\
        ...
        + (1/tau)*( f_equil_extrap(f_n, f_nM1, idx) + f_equil(f_n, idx) ) * fe.dot( xi[idx], fe.grad(v) )\
                    + 0.5*(body_force_np1 + body_force_n)*alpha_plus*v\
                        + 0.5*(body_force_np1 + body_force_n)\
                            *fe.dot( xi[idx], fe.grad(v) )\
        -alpha_plus*fe.dot( xi[idx], fe.grad(f_n[idx]) )*fe.dot( xi[idx], fe.grad(v) )\
            - f_n[idx]* fe.dot( xi[idx], fe.grad(v) ) )*fe.dx\
        
    
    lin_form_surf_int = - alpha_plus*v*w[idx]*rho_wall*fe.dot( xi[idx], fe.FacetNormal(mesh) ) * fe.ds\
        - v*w[idx]*rho_wall*fe.dot( xi[idx], fe.FacetNormal(mesh) ) * fe.ds 
        
    
    linear_forms_step2.append( lin_form_vol_int + lin_form_surf_int )  
...
for n in range(1, num_steps):
    ...
    if n%10 == 0:
        print('steps = %.4f: error = %.3g' % (n, error))
...
plt.xlabel("u_x")
plt.ylabel("y")
title_str = f"Velocity profile, tau = {tau}, L_x = {L_x}"
plt.title(title_str)
plt.show()
