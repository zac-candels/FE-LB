...
WORKDIR = os.getcwd()
outDirName = os.path.join(WORKDIR, "matMultAssembly") #f"theta{theta_deg}_tau{tau}_A{A}_kappa{kappa}_M{M_tilde}") #f"figures_CA{theta_deg}")
...
vel_star=fe.Function(Vvec_continuous)
mu_n = fe.Function(V)
rho_n = fe.Function(V)
...
for n in range(num_steps):
    ...
    velExpr = getVel(f_star, force_density)
    
    fe.project(velExpr, Vvec_continuous, function=vel_star)
    ...
    bc_f5.apply(sys_mat[5], rhsVecStreaming[5])
    bc_f2.apply(sys_mat[2], rhsVecStreaming[2])
    bc_f6.apply(sys_mat[6], rhsVecStreaming[6])

    # # Apply BCs for distribution functions 7, 4, 8
    bc_f7.apply(sys_mat[7], rhsVecStreaming[7])
    bc_f4.apply(sys_mat[4], rhsVecStreaming[4])
    bc_f8.apply(sys_mat[8], rhsVecStreaming[8])
