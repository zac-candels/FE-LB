...
f_star_coll = []
for idx in range(Q):
    f_star_coll.append(fe.Function(V))
    
f_star_stream = []
for idx in range(Q):
    f_star_stream.append(fe.Function(V))
...
streamingPrevTimeVecs= [f_star_coll[0].vector().copy() for _ in range(Q)]
advectionVecs = [f_star_coll[0].vector().copy() for _ in range(Q)]
doubleAdvectionVecs =[f_star_coll[0].vector().copy() for _ in range(Q)]
rhsVecStreaming = [f_star_coll[0].vector().copy() for _ in range(Q)]

forceVec_x = f_star_coll[0].vector().copy()
forceVec_y = f_star_coll[0].vector().copy()
...
f_starVec = f_star_coll[0].vector().copy()
...
for n in range(num_steps):
    ...
    for idx in range(Q):
        M_lumped.mult(f_star_coll[idx].vector(), streamingPrevTimeVecs[idx])
        advectionMats[idx].mult(f_star_coll[idx].vector(), advectionVecs[idx])
        doubleAdvectionMats[idx].mult(f_star_coll[idx].vector(), doubleAdvectionVecs[idx])
    ...
    f5_lower_func.assign(f_star_coll[7] )
    f2_lower_func.assign(f_star_coll[4] )
    f6_lower_func.assign(f_star_coll[8] )
    f7_upper_func.assign(f_star_coll[5] )
    f4_upper_func.assign(f_star_coll[2] )
    f8_upper_func.assign(f_star_coll[6] )
