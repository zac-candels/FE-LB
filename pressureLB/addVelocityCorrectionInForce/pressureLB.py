...
def getVel(f_list, phi, forceDensity):
    ...
    density = getDens(phi)
    velocity = distr_fn_sum/(density*c_s**2) - forceDensity * dt / 2
...
def Gamma_vel(f_list, phi, vel_idx):
    vel_arg = getVel(f_list, phi, forceDensity)
    ci = xi[vel_idx]
...
def body_Force(f_list, phi, mu, forceDensity, vel_idx):
    
    fluid_vel = getVel(f_list, phi, forceDensity)
    density = getDens(phi)
    ...
    term2 = forceDensity * Gamma_vel(f_list, phi, vel_idx)
    

    return fe.dot( velocity_prefactor, term1 + term2 )


forceDensity = -phi_n * fe.grad(mu_n)
...
lin_form_AC = phi_n * v * fe.dx - dt*v*fe.dot(getVel(f_n, phi_n, forceDensity), fe.grad(phi_n))*fe.dx\
    - dt*M_tilde*v*mu_n*fe.dx - (beta_mass_diff/dt)*mass_diff*fe.sqrt( fe.dot(fe.grad(phi_n), fe.grad(phi_n)) )*v*fe.dx\
        - 0.5*dt**2 * fe.dot(getVel(f_n, phi_n, forceDensity), fe.grad(v)) * fe.dot(getVel(f_n, phi_n, forceDensity), fe.grad(phi_n)) *fe.dx
...
for idx in range(Q):
    ...
    dot_product_force_term = 0.5*dt**2 * fe.dot(xi[idx], fe.grad(v))\
        * body_Force(f_star, phi_n, mu_n, forceDensity, idx) * fe.dx
    ...
    lin_form_idx = f_star[idx]*v*fe.dx\
        ...
        + dt*v*body_Force(f_star, phi_n, mu_n, forceDensity, idx)*fe.dx\
...
for n in range(num_steps):
    ...
    if 1 == 1:
        if n % 200 == 0:  # plot every 10 steps
            ...
            vel_expr = getVel(f_n, phi_n, forceDensity)
