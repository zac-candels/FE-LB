...
WORKDIR = os.getcwd()
outDirName = os.path.join(WORKDIR, f"test") #f"theta{theta_deg}_tau{tau}_A{A}_kappa{kappa}_M{M_tilde}") #f"figures_CA{theta_deg}")
...
bilin_form_AC = phi_trial * q * fe.dx
...
for idx in range(Q):
    ...
    lin_form_coll = f_equil(f_n, idx, force_density)*v*fe.dx
...
collisionPrevTimeVecs= [f_star[0].vector().copy() for _ in range(Q)]
...
for n in range(num_steps):
    ...
    for idx in range(Q):
        equilDistrTerm = fe.assemble(linear_forms_collision[idx])
        M_lumped.mult(f_n[idx].vector(), collisionPrevTimeVecs[idx])
        rhs_vec_collision[idx] = collisionPrevTimeVecs[idx]
        rhs_vec_collision[idx].axpy(-dt/tau, collisionPrevTimeVecs[idx])
        rhs_vec_collision[idx].axpy(dt/tau, equilDistrTerm)

        
    for idx in range(Q):
        vi = fe.as_backend_type(rhs_vec_collision[idx]).vec()
        f_star[idx].vector().vec().pointwiseDivide(vi, sysMatLumped[idx])
