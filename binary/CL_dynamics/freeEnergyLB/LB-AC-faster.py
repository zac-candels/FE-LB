...
def getPres(f_list, phi, mu):
    
    distr_sum = f_list[0] + f_list[1] + f_list[2] + f_list[3] + f_list[4]\
        + f_list[5] + f_list[6] + f_list[7] + f_list[8]
        
    vel = getVel(f_list, phi, mu)
    density = getDens(phi)
    
    vel_correction = c_s2*fe.dot( vel, fe.grad(density) )
    
    return distr_sum + (dt/2)*vel_correction

# Define velocity

def getVel(f_list, phi, mu):
    ...
    body_force = phi*fe.grad(mu)
    density = getDens(phi)
    velocity = ( distr_fn_sum/c_s**2 - (dt/2)*body_force )/density
...
def f_equil(f_list, phi_n, mu_n, vel_idx):
    pres = sum(fj for fj in f_list)
    rho_expr = getDens(phi_n)
    u_expr   = getVel(f_list, phi_n, mu_n)    
    ci       = xi[vel_idx]
...
# Define \Gamma
def Gamma_vel(f_list, phi, mu, vel_idx):
    vel_arg = getVel(f_list, phi, mu)
    ci = xi[vel_idx]
...
def body_Force(f_list, phi, mu, vel_idx):
    ...
    fluid_vel = getVel(f_list, phi, mu)
    density = getDens(phi)
    
    velocity_prefactor = xi[vel_idx] - fluid_vel
    
    term1 = fe.grad( density* c_s2 )*( Gamma_vel(f_list, phi, mu, vel_idx)\
                                     - Gamma0(vel_idx) )
    
    term2 = mu * fe.grad(phi) * Gamma_vel(f_list, phi, mu, vel_idx)
...
lin_form_AC = phi_n * v * fe.dx - dt*v*fe.dot(getVel(f_n, phi_n, mu_n), fe.grad(phi_n))*fe.dx\
    - dt*(1/Pe)*v*mu_n*fe.dx - (beta_mass_diff/dt)*mass_diff*fe.sqrt( fe.dot(fe.grad(phi_n), fe.grad(phi_n)) )*v*fe.dx\
        - 0.5*dt**2 * fe.dot(getVel(f_n, phi_n, mu_n), fe.grad(v)) * fe.dot(getVel(f_n, phi_n, mu_n), fe.grad(phi_n)) *fe.dx
...
for idx in range(Q):
    ...
    lin_form_coll = (f_n[idx] - 1/(getTau(phi_n) ) * (f_n[idx] - f_equil(f_n, phi_n, mu_n, idx)))*v*fe.dx
...
for n in range(num_steps):
    ...
    if 1 == 1:
        if n % 10 == 0:  # plot every 10 steps
            ...
            vel_expr = getVel(f_n, phi_n, mu_n)
