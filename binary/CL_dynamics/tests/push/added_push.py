...
CFL = 0.2
initBubbleDiam = 5
L_x, L_y = 2*initBubbleDiam, 1*initBubbleDiam
nx, ny = 100, 100
h = min(L_x/nx, L_y/ny)
dt = h*CFL
...
WORKDIR = os.getcwd()
outDirName = os.path.join(WORKDIR, f"push_CA{theta_deg}")
...
def body_Force(f_list, phi, mu, vel_idx):
    ...
    term5 = Gamma_vel(fluid_vel, vel_idx)\
        *fe.dot( (xi[vel_idx] - fluid_vel), grav ) / density
